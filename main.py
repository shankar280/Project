...
def assure_path_exists(path):
    ...
    if not os.path.exists(dir):
        os.makedirs(dir)
def contact():
    mess._show(title='Contact', message="You can contact me on : 'shankarchakraborty280@gmail.com' ")
...
def check_haarcascadefile():
    ...
    if exists:
        ...
        mess._show(title='file is missing', message='For any help, feel free to contact')
        window.destroy()
...
def save_pass():
    ...
    if exists1:
        ...
        master.destroy()
        new_pas = tsd.askstring('Old Password was not found!', 'Please enter a new password here', show='*')
        if new_pas == None:
            mess._show(title='Nothing Entered', message='Password is not updated! Kindly try again')
        else:
            tf = open("TrainingImageLabel\psd.txt", "w")
            tf.write(new_pas)
            mess._show(title='Password Updated', message='New password has been registered successfully!!')
            return
    op = (old.get())
    newp= (new.get())
    nnewp = (nnew.get())
...
def TrackImages():
    ...
    assure_path_exists("Person Details/")
    for k in tv.get_children():
        tv.delete(k)                #remove all the entries from the tree view
    msg = ''
    i = 0
    j = 0
...
window.title("Facial recognition based entry  System")
...
message3 = tk.Label(window, text="Facial Recognition Based Entry System" ,fg="white",bg="black" ,width=55 ,height=1,font=('Monotype Corsiva', 29, ' bold '))
message3.place(x=100, y=10)

frame3 = tk.Frame(window, bg="#c4c6ce")
frame3.place(relx=0.8, rely=0.1, relwidth=0.09, relheight=0.07)

frame4 = tk.Frame(window, bg="#c4c6ce")
frame4.place(relx=0.11, rely=0.10, relwidth=0.25, relheight=0.07)

datef = tk.Label(frame4, text = day+"-"+mont[month]+"-"+year+"  |  ", fg="white",bg="#2d420a" ,width=55 ,height=1,font=('white', 21, ' bold '))
datef.pack(fill='both',expand=1)

clock = tk.Label(frame3,fg="orange",bg="gray" ,width=55 ,height=1,font=('comic', 18, ' bold '))
...
trackImg = tk.Button(frame1, text="Take Attendance", command=TrackImages  ,fg="white"  ,bg="green"  ,width=35  ,height=1, activebackground = "white" ,font=('comic', 15, ' bold '))
